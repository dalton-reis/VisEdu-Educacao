        ...
        if (rb_Atual.getTipo() === "Esfera") {
            ...
            if (rb_Atual.getColisaoCom().length > 0) {
                ...
                for (var i = 0; i < rb_Atual.getColisaoCom().length; i++) {
                    ...
                    if ((rb_Fis.getTipo() === "Esfera")) {
                        //alert("Entrou Aqui!");
                        Esfera_Esfera(rb, rb_ColideCom, rb_Fis, rb_Atual.getMaterial());
                    } else {
                        if ((rb_Fis.getTipo() === "Cubo") || (rb_Fis.getTipo() === "Alvo")) {
            ...
            if ((rb_Atual.getTipo() === "Cubo") || (rb_Atual.getTipo() === "Alvo")) {
                if (rb_Atual.getTipo() === "Cubo") {
                    ...
                    rb.halfSize = new THREE.Vector3((rb_Atual.getTamanho() / 2), rb_Atual.getTamanho(), rb_Atual.getTamanho());
                }
                rb.inertiaTensor.set(960000.0381469727, -0.0, -0.0, -0.0, 960000.0381469727, -0.0, -0.0, -0.0, 960000.0381469727);
                console.log(rb.inertiaTensor);
                simulation.bindRigidBody(rb);
                listaCorposRigidos.push(rb);
                if (rb_Atual.isColideComBase()) {
                    colisaoBase_Cubo(rb);
