...
ItemEditorObjetoGrafico = function() {
	...
	var generateDimanteOG = function() {
	...
	var generateQuadradoOG = function() {
	...
	scope.onAddFilho = function ( item ) {
		...
		if (item.tipoEncaixe == ETiposEncaixe.SETA) {
			//se adicionada a animação, habilita a flag
			if( item.id == EIdsItens.ANIMACAO ){
	...
	scope.onRemoveFilho = function ( item ) {
		//se removido a animação, desliga a flag
		if( item.id == EIdsItens.ANIMACAO ){
				scope.isAnimated = false;
		}
