...
(function ($) {
    ...
    var EasyTree = function (jQueryContext, options) {
        ...
        this.addBrotherNode = function (sourceNode, targetId) {
            ...
            var result = {};
            var targetNode = getParentNode(_nodes, targetId, result);
            if (targetNode) {
              targetNode = result.parent;
            }
        ...
        function getParentNode(nodes, id, result) {
            var i = 0;
            for (i = 0; i < nodes.length; i++) {
                var n = nodes[i];
                var t = n.text;
